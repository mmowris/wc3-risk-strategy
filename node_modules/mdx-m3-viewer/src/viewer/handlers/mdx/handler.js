...
exports.default = {
    load(viewer, pathSolver, reforged = false) {
        ...
        const sdShader = webgl.createShader(standard_vert_1.default, standard_frag_1.default);
        const sdExtendedShader = webgl.createShader(sdExtendedVert, standard_frag_1.default);
        ...
        let shaders = [];
        shaders[viewer_1.DebugRenderMode.Diffuse] = webgl.createShader(standard_vert_1.default, sdDiffuse);
        shaders[viewer_1.DebugRenderMode.TexCoords] = webgl.createShader(standard_vert_1.default, sdTexcoords);
        shaders[viewer_1.DebugRenderMode.Normals] = webgl.createShader(standard_vert_1.default, sdNormals);
    ...
    loadTeamTextures(viewer) {
        ...
        if (teamColors.length === 0) {
            ...
            for (let i = 0; i < teams; i++) {
                ...
                const end = `${id}.${ext}`;
                const teamColor = new texture_1.default(1, true, true);
                const teamGlow = new texture_1.default(2, true, true);
    ...
    getEventObjectSoundFile(file, reforged, isHd, tables) {
        ...
        for (let i = 1, l = tables.length; i < l; i++) {
            ...
            if (raceRow) {
                const flags = raceRow['Flags'];
                const filePath = raceRow['Filepath'];
                if (flags === 'SD_ONLY') {
                    if (!isHd) {
                        return filePath;
                    }
    ...
    async getEventObjectData(viewer, type, id, isHd) {
        ...
        if (type === 'SND') {
            ...
            if (reforged) {
                ...
                if (lookupRow) {
                    row = mappedData.getRow(lookupRow['SoundLabel']);
                }
            }
            if (row) {
                for (const fileName of row['FileNames'].split(',')) {
            ...
            if (row) {
                if (type === 'SPN') {
                    promises.push(viewer.load(row['Model'].replace('.mdl', '.mdx'), safePathSolver, params));
                }
